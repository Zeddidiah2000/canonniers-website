# Directive: Photo-Vault Architecture & Security Protocol (v4.2 — Implementation-Ready)

> **Changes from v4.1:** Adds `event_type` column to support practice, team event, tournament, and "other" photos in addition to league games. Match Picker becomes conditional (games only). Gallery gains a secondary filter. Adds content-policy note for non-game photos. No change to auth, upload flow, Cloudflare Images integration, or rate limiting.
...
**Scope:** Phase 1 only. Supports five event types:

- `game` — league games sourced from Spordle via Match Picker (auto-fill metadata)
- `practice` — team practices (manual date + location)
- `team_event` — banquets, photo days, fundraisers, awards nights
- `tournament` — multi-day tournament groupings
- `other` — escape hatch for uncategorized content

Fan Zone (public contributions), Story Generator, and post-game result cards are explicitly deferred to Phase 2+.

**Roles with access:** `admin`, `coach`, `manager`. Locked for `treasurer`.

**Content responsibility:** Coaches and managers are responsible for obtaining parental consent before uploading non-game content (practices, locker room/dugout candids, travel photos, team events). The site has no consent management system in Phase 1. JP and Jay carry this responsibility, not the codebase. This is not a technical control; it is a policy reminder.
...
**Notes:**
- `event_type` defaults to `'game'` — backward-safe for any insert that omits the column.
- `event_name_fr` semantics by event type:
  - `game` → auto-filled from Spordle, format: `vs <opponent>` (e.g., `vs Voyageurs de Saguenay`)
  - `practice` → free text, recommended format: `Pratique - <location>` (e.g., `Pratique - Stade Canac`)
  - `team_event` → free text describing the event (e.g., `Banquet de fin de saison 2026`)
  - `tournament` → free text with tournament name (e.g., `Tournoi de Sherbrooke - Jour 1`)
  - `other` → free text, uploader's choice
...
- `event_name_fr` is FR-only by deliberate design (consistent with québécois register; opponent names from Spordle are French; manual entries should also be in French).
- The new `idx_photos_team_type_date` index supports the gallery's secondary filter (team × event type × date) without scanning the full table.
...
| `GET` | `/api/photos?team=u15&type=game&from=2026-04-01&to=2026-04-30` | Public read endpoint. Filters: `team` (required), `type` (optional, omit for all types), `from`, `to`. **Always** appends `WHERE is_published = 1`. No auth required. |
...
### 8.3 Upload Form Flow

The form follows a three-step flow with adaptive metadata fields based on event type.

**Step 1: Team selection.**
Three-button segmented control (15U AAA / 17U D1 / 17U D2). Persisted via `localStorage` key `canonniers_photo_last_team` so reuploading from the same team doesn't require re-selecting. Required before Step 2 is shown.

**Step 2: Event type selection.**
Five-button segmented control with bilingual labels:
- `game` — "Match" / "Game"
- `practice` — "Pratique" / "Practice"
- `team_event` — "Événement d'équipe" / "Team Event"
- `tournament` — "Tournoi" / "Tournament"
- `other` — "Autre" / "Other"

Default: `game` (most common case). The form below adapts based on selection.

**Step 3a: If `event_type === 'game'` — Match Picker.**

1. On Step 2 selection, fetch schedule for the selected team only (not all three) from `https://spordle-proxy.chisholm2000.workers.dev`.
2. Cache response in `localStorage` with 15-minute TTL. Key: `canonniers_schedule_cache_<team>` (one cache entry per team). Value: `{ data: <response>, expires: <epoch ms> }`. Check expiry on each load.
3. Render a list of recent + upcoming games (default window: last 14 days + next 7 days).
4. Show a "Show older games" toggle that extends the window to the last 60 days when activated. Toggle state is not persisted between sessions.
5. On game selection, auto-fill `event_date` and `event_name_fr` (formatted as `vs <opponent>` from Spordle data; use the same opponent name format the public calendar already uses).
6. `event_name_fr` and `event_date` fields are read-only in this mode. To override, the uploader must switch to `event_type: 'other'`.

**Step 3b: If `event_type !== 'game'` — Manual entry.**

The Match Picker hides. Two fields appear:
- **Date** (`event_date`): native `<input type="date">`, required, must be ≤ today + 30 days (prevents typos like `9999-12-31`).
- **Event name** (`event_name_fr`): native `<input type="text">`, required, max 100 characters, FR-only. Placeholder text suggests the format per `event_type`:
  - `practice` → `ex: Pratique - Stade Canac`
  - `team_event` → `ex: Banquet de fin de saison 2026`
  - `tournament` → `ex: Tournoi de Sherbrooke - Jour 1`
  - `other` → `ex: Photo d'équipe officielle`

No Spordle call is made for non-game uploads.
...
- **Submit button enabled only when:** team selected AND event type selected AND (game selected OR (`event_date` filled AND `event_name_fr` non-empty)) AND at least one valid file in the queue.
...
- **Primary filter (tabs):** Three-tab structure (15U AAA, 17U D1, 17U D2) consistent with calendrier/diffusion/alignement.
- **Secondary filter (pills under tabs):** Event type filter with bilingual labels. Options: "Tous / All" (default), "Matchs / Games", "Pratiques / Practices", "Événements / Team Events", "Tournois / Tournaments", "Autres / Other". Pill-style buttons matching the existing roster filter UI.
- Within the active filter combination: photos grouped by `event_date` descending, then by `event_name_fr`.
...
- The secondary filter selection is persisted in `localStorage` key `canonniers_gallery_filter` for the current session only (resets on browser close).
- If a team + event type combination has zero photos, show an empty state for that combination (not a blank screen).
...
- Upload one test photo of each format (JPEG, PNG, WebP) using `event_type: 'game'` flow → confirm Match Picker auto-fill works
- Upload one photo using `event_type: 'practice'` with manual date + name → confirm row inserts with `event_type = 'practice'`
- Upload one photo using `event_type: 'team_event'` → confirm row inserts with `event_type = 'team_event'`
- Try submitting with empty `event_name_fr` in non-game flow → expect form validation block
...
- Confirm D1 rows have correct `event_type` value matching the flow used
- Confirm D1 rows have all metadata populated (width, height, mime_type non-NULL)
...
- Confirm "Tous / All" filter shows all event types for the selected team
- Confirm "Matchs / Games" filter shows only `event_type = 'game'` photos
- Confirm filtering by an event type with zero photos shows the empty state, not a blank screen
...
*End of directive. v4.2 — drafted 2026-04-29. Adds `event_type` column and conditional Match Picker. Roles locked: admin + coach + manager. Pre-flight values resolved.*
